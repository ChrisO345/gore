...
func NotEqual[T comparable](t testing.TB, actual, unexpected T) {
	...
	if unexpected == actual {
		t.Fatalf("%s", getErrorMessage(unexpected, actual))
	}
}
...
func True(t testing.TB, condition bool) {
	t.Helper()
	if !condition {
		t.Fatalf("%s", getErrorMessage(true, condition))
	}
}
...
func False(t testing.TB, condition bool) {
	t.Helper()
	if condition {
		t.Fatalf("%s", getErrorMessage(false, condition))
	}
}
...
func Zero[T numeric](t testing.TB, v T) {
	t.Helper()
	if v != 0 {
		t.Fatalf("%s", getErrorMessage(0, v))
	}
}
...
func NotZero[T numeric](t testing.TB, v T) {
	t.Helper()
	if v == 0 {
		t.Fatalf("%s", getErrorMessage("not zero", v))
	}
}
...
func Nil(t testing.TB, value any) {
	t.Helper()
	if value != nil {
		t.Fatalf("%s", getErrorMessage(nil, value))
	}
}
...
func NotNil(t testing.TB, value any) {
	t.Helper()
	if value == nil {
		t.Fatalf("%s", getErrorMessage("not nil", "nil"))
	}
}
...
func Panic(t testing.TB, f func()) {
	...
	defer func() {
		r := recover()
		if r == nil {
			t.Fatalf("%s", getErrorMessage("panic", "no panic occurred"))
		}
	}()
	f()
}
...
func Safe(t testing.TB, f func()) {
	...
	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("%s", getErrorMessage("no panic", "panic occurred"))
		}
	}()
	f()
}
...
func Length[T container](t testing.TB, collection T, expectedLength int) {
	...
	if actualLength != expectedLength {
		t.Fatalf("%s", getErrorMessage(expectedLength, actualLength))
	}
}
...
func PrefixString(t testing.TB, str, prefix string) {
	...
	if !hasPrefix(str, prefix) {
		t.Fatalf("%s", getErrorMessage(prefix, str))
	}
}
...
func NotPrefixString(t testing.TB, str, prefix string) {
	t.Helper()
	if hasPrefix(str, prefix) {
		t.Fatalf("%s", getErrorMessage("not prefix", str))
	}
}
...
func SuffixString(t testing.TB, str, suffix string) {
	...
	if !hasSuffix(str, suffix) {
		t.Fatalf("%s", getErrorMessage(suffix, str))
	}
}
...
func NotSuffixString(t testing.TB, str, suffix string) {
	t.Helper()
	if hasSuffix(str, suffix) {
		t.Fatalf("%s", getErrorMessage("not suffix", str))
	}
}
